...
/**
 * Returns true if the loading screen should show.
 * The loading screen shows once per 12-hour window.
 * After it completes, `loadingScreenComplete` is set to the current timestamp.
 * If less than 12 hours have passed since that timestamp, the screen is skipped.
 */
function shouldShowLoadingScreen(completedTimestamp) {
  if (!completedTimestamp) return true; // never completed before
...
const useStore = create(
  persist(
    (set, get) => ({
      ...
      loadingScreenComplete: null, // timestamp when loading screen last finished

      // Actions
      completeLoadingScreen: () => set({ loadingScreenComplete: Date.now() }),

      shouldShowLoadingScreen: () => shouldShowLoadingScreen(get().loadingScreenComplete),
